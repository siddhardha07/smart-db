...
import { Upload, FileJson, Send, CheckCircle, AlertCircle, Loader, Table, Info } from 'lucide-react';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setResponseMessage(`Data uploaded successfully! ${result.insertedRecords} records inserted.`);
      } else {
        setUploadState('error');
      ...
      setUploadState('error');
      setResponseMessage(error instanceof Error ? error.message : 'Failed to upload data');
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-full">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Insert JSON data into {database.name}</p>
        </div>
      </div>
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2">
              {database.tables.map(table => (
                <div key={table.name} className="bg-white rounded p-3 border border-blue-100">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Provide JSON data with table names as keys and arrays of records as values. IDs will be auto-generated.
        </p>
      </div>
      ...
      {responseMessage && (
        <div className={`p-4 rounded-xl flex items-start gap-3 mb-6 ${
          uploadState === 'success' ? 'bg-green-50 border border-green-200' :
          uploadState === 'error' ? 'bg-red-50 border border-red-200' :
          'bg-blue-50 border border-blue-200'
        }`}>
          {uploadState === 'success' && <CheckCircle className="w-5 h-5 text-green-500 mt-0.5" />}
          {uploadState === 'error' && <AlertCircle className="w-5 h-5 text-red-500 mt-0.5" />}
          <p className={`text-sm font-medium ${
            uploadState === 'success' ? 'text-green-800' :
            uploadState === 'error' ? 'text-red-800' :
            'text-blue-800'
          }`}>
            {responseMessage}
          </p>
